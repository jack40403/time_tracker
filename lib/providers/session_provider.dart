...
class SessionsNotifier extends Notifier<List<TimeSession>> {
  String _sessionKey(TimeSession session) {
    return session.id.isNotEmpty ? session.id : TimeSession.generateId(session.category, session.date);
  }

  bool _isSameExecutionSegment(TimeSession a, TimeSession b) {
    final sameCategory = TimeSession.toBaseName(a.category) == TimeSession.toBaseName(b.category);
    final sameDuration = a.durationSeconds == b.durationSeconds;
    final aBucket = a.date.toUtc().millisecondsSinceEpoch ~/ 5000;
    final bBucket = b.date.toUtc().millisecondsSinceEpoch ~/ 5000;
    return sameCategory && sameDuration && aBucket == bBucket;
  }

  List<TimeSession> _dedupeSessions(List<TimeSession> sessions) {
    final deduped = <TimeSession>[];
    for (final session in sessions) {
      final index = deduped.indexWhere((existing) =>
          existing.id == session.id || _isSameExecutionSegment(existing, session));
      ...
      final shouldReplace = (existing.note == null || existing.note!.isEmpty) &&
  ...
  Future<int> forceCloudCleanup() async {
    ...
    try {
      ...
      for (var s in cloudData) {
        final session = TimeSession.fromJson(Map<String, dynamic>.from(s));
        final stableId = _sessionKey(session);
  ...
  Future<void> _syncWithCloud(List<dynamic> cloudSessions) async {
    ...
    for (var s in cloudSessions) {
      ...
      final stableId = _sessionKey(session);
      if (cloudDocId.isEmpty) {
        zombieIds.add(stableId);
      }
    ...
    final dedupedSessions = _dedupeSessions(finalSessions);
    
    // Update state and save
    state = dedupedSessions;
    _saveLocally(state);
    ...
    if (finalSessions.length < cloudSessions.length || zombieIds.isNotEmpty) {
      ...
      if (firestore != null) {
        ...
        await firestore.batchUploadSessions(dedupedSessions);
      }
    }
  ...
  Future<int> importSessions(List<TimeSession> newSessions) async {
    ...
    final currentMap = {
      for (var s in state) _sessionKey(s): s
    };

    int importedCount = 0;
    for (var s in newSessions) {
      final id = _sessionKey(s);
  ...
  Future<void> updateSession(TimeSession updated) async {
    state = state.map((s) => s.id == updated.id ? updated : s).toList();
    _saveLocally(state);
    final firestore = ref.read(firestoreServiceProvider);
    if (firestore != null) await firestore.updateSession(updated);
  }
  ...
  Future<void> addSession(TimeSession session) async {
    ...
    final next = [stableSession, ...state];
    state = _dedupeSessions(next);
    _saveLocally(state);
    final firestore = ref.read(firestoreServiceProvider);
    if (firestore != null) {
      final mergedSession = state.firstWhere(
        (s) => s.id == stableSession.id,
        orElse: () => stableSession,
      );
      await firestore.addSession(mergedSession);
    }
  }
